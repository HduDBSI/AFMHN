...
model_dm = Doc2Vec.load("../result/doc2vec_new.model")

# 读取
with open('../result/samples_new.pickle', 'rb') as f:
...
all_columns = [label_column, API_doc2vec_columns, Mashup_doc2vec_columns, API_Mashup_similarity_column,
               API_popularity_column, API_category_column, Mashup_category_column]
...
for index, sample in enumerate(samples):  # (API,Mashup,label)
    ...
    df.loc[index, API_doc2vec_columns] = API_doc2vec_values
    Mashup_doc2vec_values = model_dm.infer_vector(sample[1]["Desc"])  # Mashup doc2vec
    df.loc[index, Mashup_doc2vec_columns] = Mashup_doc2vec_values
    df.loc[index, API_Mashup_similarity_column] = model_dm.dv.similarity(sample[0]["tags_no"], sample[1][
        "tags_no"])  # cos_similarity(API_doc2vec_values,Mashup_doc2vec_values)
    df.loc[index, API_popularity_column] = sample[0]["popularity"]
    df.loc[index, API_category_column] = sample[0]["Primary Category"]
    df.loc[index, Mashup_category_column] = sample[1]["Categories"][0]

# save df to csv
df.to_csv("../result/input_data_new.csv", index=False, float_format='%.9f')
