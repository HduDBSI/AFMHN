...
from deepctr_torch.models import basicFM, DeepFM, AFM, NAFM, xDeepFM, AutoInt, xDeepAFM, xDeepMAFM
...
if __name__ == "__main__":
    ...
    data = pd.read_csv('../result/input_data_sen.csv')
    ...
    [dnn_hidden_units, attention_factor, cin_layer_size, l2_reg, dropout, batch_size] = [8, 32, 8, 0.1, 0.5, 64]
    # [dnn_hidden_units, attention_factor, cin_layer_size, l2_reg, dropout, batch_size] = [2, 4, 4, 0.1, 0.5, 64]
    df = pd.DataFrame(np.zeros((7, 8)), index=["basicFM", "DeepFM", "AutoInt", "xDeepFM", "AFM", "NAFM", "xDeepAFM"],
    ...
    for test_size in [0.2, 0.4, 0.6, 0.8]:
        ...
        for i in range(5):
            model = xDeepAFM(linear_feature_columns, dnn_feature_columns, attention_factor=attention_factor,
                             dnn_hidden_units=(dnn_hidden_units, dnn_hidden_units),
                             cin_layer_size=(cin_layer_size, cin_layer_size),
                             l2_reg_linear=l2_reg, l2_reg_embedding=l2_reg, l2_reg_att=l2_reg, l2_reg_cin=l2_reg,
                             l2_reg_dnn=l2_reg,
    ...
    df.to_csv("../result/evaluation_results_1103.csv", index=True, float_format='%.4f')
    print(df)
