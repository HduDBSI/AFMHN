...
if __name__ == "__main__":
    ...
    p_attention_factor = [32, 16, 8, 64, 128, 256]
                    ...
                    for dropout in p_dropout:
                        for batch_size in P_batch_size:
                            all_paramaters.append(
                                (dnn_hidden_units, attention_factor, l2_reg, dropout, batch_size))
    best_loss = np.inf;
    best_auc = 0
    for index, (dnn_hidden_units, attention_factor, l2_reg, dropout, batch_size) in enumerate(
            all_paramaters):
        model = AFN(linear_feature_columns, dnn_feature_columns,
                        ltl_hidden_size=attention_factor,
                         afn_dnn_hidden_units=(dnn_hidden_units, dnn_hidden_units),
