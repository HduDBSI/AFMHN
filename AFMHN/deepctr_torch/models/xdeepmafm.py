...
from ..layers import FM, AFMLayer, InteractingLayer, concat_fun


class xDeepMAFM(BaseModel):
    ...
    def __init__(self, linear_feature_columns, dnn_feature_columns, dnn_hidden_units=(256, 256),
                 use_attention=True, att_head_num=2, att_res=True,att_layer_num=3,
                 cin_layer_size=(256, 128,), cin_split_half=True, cin_activation='relu', l2_reg_linear=0.00001,
                 bi_dropout=0,
                 l2_reg_embedding=0.00001, l2_reg_dnn=0, l2_reg_cin=0, init_std=0.0001,
                 seed=1024, dnn_dropout=0,
                 dnn_activation='relu', dnn_use_bn=False, task='binary', device='cpu', gpus=None):

        super(xDeepMAFM, self).__init__(linear_feature_columns, dnn_feature_columns, l2_reg_linear=l2_reg_linear,
        ...
        #     #                    seed, device)
        #     self.fm = InteractingLayer(self.embedding_size, att_head_num, att_res, device=device)
    ...
    def forward(self, X):
        ...
        if self.use_attention and self.att_layer_num > 0:
