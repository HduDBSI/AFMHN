        ...
        for no, row in enumerate(spamreader):
            ...
            if no == 0:
                ...
                for index, field in enumerate(csv_head_list):
                    if field == "Related APIs":
                        Called_APIs = row[index].split(",")
                        one_dict[field] = Called_APIs
                    elif field in ("Secondary Categories", "Categories"):
...
def Doc2Vec_train(x_train, size=100, dm=1):  # size=100 意味着每个词向量是100维的
    ...
    model_dm.save("../result/doc2vec_new.model")
    return model_dm
...
def popuparity(Mashups=None, APIs=None):
    ...
    for Mashup in Mashups:
        invocations.extend(Mashup["Related APIs"])
    ...
    for API in APIs:
        ...
        frequence_list = category_frequences.get(API["Primary Category"], [])
        frequence_list.append(API["frequence"])
        category_frequences[API["Primary Category"]] = frequence_list
        MemberMashups = []
        for Mashup in Mashups:
            if API["APIName"] in Mashup["Related APIs"]:
                MemberMashups.append(Mashup["MashupName"])
        API["MemberMashups"] = MemberMashups

    for API in APIs:
        max_fre = max(category_frequences[API["Primary Category"]])
        min_fre = min(category_frequences[API["Primary Category"]])
        if max_fre == 0:
            API["popularity"] = 0
        else:
...
if __name__ == "__main__":
    ...
    Mashup_file_path = os.path.join(home_path, '../dataset_new/mashup.csv')
    API_file_path = os.path.join(home_path, '../dataset_new/api.csv')
    ...
    # print("A sample of APIs: ", APIs[0])
    popular_APIs, popular_Mashups = get_popular_APIs_Mashups(Mashups, APIs, top_k=150)
    print("cal end...")
    ...
    categories = []
    for API in popular_APIs:
        categories.append(API["Primary Category"])
    for Mashup in popular_Mashups:
        categories.append(Mashup["Categories"][0])
    ...
    samples = get_samples(popular_Mashups, popular_APIs, sample_number=10000)
    print("generate end...")
    ...
    # 保存
    with open('../result/samples_new.pickle', 'wb') as f:
